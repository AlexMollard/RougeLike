...
public class MapGen : MonoBehaviour
{
    ...
    bool IsNonGroundTile(int x, int y)
    {
        for (int i = 0; i < nonGroundTiles.Length; i++)
        {
            if (tiles[x][y].GetComponent<TileBehaviour>().type == nonGroundTiles[i])
                return true;
        }
        return false;
    }
